...
export default {
    ...
    mounted: function() {
		UserService.getAllUserCreationRequests().then(response => {
    ...
    methods: {
        ...
        approveRequest(id) {
			UserService.approveRequest(id).then(location.reload());
        },
        rejectRequest(id, description) {
			UserService.rejectRequest(id, description).then(location.reload());
        }
    }
}
