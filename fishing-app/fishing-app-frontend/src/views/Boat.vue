<template>
    <div class="white-panel">
		<div class="register-show">
			<h2>{{boat.name}}</h2>
            <p>{{boat.address.address}} {{boat.address.city}} {{boat.address.country}}</p>
            ...
            <button class="card-button" v-on:click="reserveBoat()">View</button>
		</div>
	</div>
</template>
...
export default {
    ...
    data(){
        return {
            ...
            selectedMonthTo: 1,
            selectedDayTo: 1,
            availableBoatReservationDates: [],
            reservationParameters: {}
        }
    },
    mounted: function() {
        BoatService.getReservable(this.$route.params.id).then(res => {
          this.boat = res.data
        });
        this.user = localStorage.getItem('user');
        BoatService.getAvailableReservablesReservationDates(this.$route.params.id).then(res => {
          this.availableBoatReservationDates = res.data
        });
    ...
    methods: {
        reserveBoat() {
            ...
            BoatService.reserveReservable(this.reservationParameters)
        }
    },
    computed: {
        getAvailableReservationDays() {
            ...
            for (let i = 1; i <= maxDays; i++) {
                this.availableBoatReservationDates.forEach((value) => {
